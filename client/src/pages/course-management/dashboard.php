...
require_once ROOT . FEATURES . '/course-management/module-component.php';
...
$_SESSION['currentPage'] = 'managecourses';
$cssFiles = ['admin-nav-bar.css', "admin-top-bar.css", 'manage-users.css', 'form.css'];
createHead("Orbit | Course Management", $cssFiles);
...
<div class="page-content">
    <div class="manage-users-container">
        <div class="title-btn-wrapper">
            <span class="manage-users-header">Course List</span>
            
            <?php
            $majorList = getMajorList();
            echo "
            <div class='form-input'>
                <select id='selectMajor' name='selectMajor' class='select-input'>";

            foreach ($majorList as $value => $label) {
                if ($value == $majorID) {
                    $select = "selected";
                } else {
                    $select = "";
            ...
            <div class="add-user-btn" onclick="openModal('createModuleForm')">
                <span class="btn-text">
                    Add New Course
                </span>
                <?php
        ...
        <?php
        $moduleList = getModuleList($majorID);
        renderCourseTable($moduleList, $majorID);
        ?>
    </div>
</div>
